...
inputFileDataFrame = inputFileDataFrame.groupby(inputFileDataFrame.index).apply(lambda group: group.interpolate(method = "linear", limit_area = "inside"))

groups = inputFileDataFrame.groupby(inputFileDataFrame.index) # Get the several groups

##############################################################
# Aux stuff to do linear extrapolation when possible
...
# Estrapolate for each column of each group
for name, group in groups:
    # Curve fit each column
    for col in inputFileDataFrame.columns: 

        if(col=='dateStep'):
            continue
        
        # Get only the non-NaNs
        colWithoutNans = group[group[col].notna()]
        x = colWithoutNans['dateStep'].values
        y = colWithoutNans[col].values

        if(len(x)==0):
            col_params[col] = []
            continue

        # Fit with constant if only 1 value
        if(len(x)==1):
            col_params[col] = [0, y[0]]
            continue
    ...
    for col in inputFileDataFrame.columns:
        if(col=='dateStep' or len(col_params[col])==0):
            continue

        # Get the index values for NaNs in the column
        x = group[pd.isnull(group[col])]['dateStep'].astype(float).values
        
        if(len(x)==0):
            continue

        # Extrapolate the points with the fitted function
        valsToFill = func(x, *col_params[col])
        k = 0
        for value in valsToFill:
